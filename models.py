...
class DeviceChange:
	...
	def sql(self):
		return 'insert into device_change_log values("%s","%s","%s",%s,"%s","%s","%s","%s","%s")' % (self.log_date, self.name, self.number,self.count, self.unit, self.old_location, self.new_location, self.old_user, self.new_user)
...
class DeviceInfo:
	...
	def sql(self):
		...
		return 'insert ignore into device_info values ("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s")' % (
			self.number,
			self.name,
